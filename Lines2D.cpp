...
const img::EasyImage Lines2D::drawLines2D(const listWithLines &list, int size) {
    double xmin=INT64_MAX; double xmax=INT64_MIN; double ymin=INT64_MAX; double ymax=INT64_MIN;
    ...
    double imagex=0;double imagey=0;double rangex=0;double rangey=0;
    ...
    for (const Line2D line: list) {
        image.draw_line(roundToInt((line.p1.x * d) + dx), roundToInt((line.p1.y * d) + dy), roundToInt((line.p2.x * d) + dx), roundToInt((line.p2.y * d) + dy), line.color);
    }
    std::cout << "i printed one" << std::endl;
    return image;
}
...
Lines2D::Lines2D(const ini::Configuration &conf) {
    ...
    cur.x = 0;cur.y = 0;prev.x = 0;prev.y = 0;
...
void Lines2D::calculateLines(const std::string &input) {
    for (char c:input) {
        ...
        else if (c == ')') {cur = stackPoint.front().second; prev = cur; currentAngle = stackPoint.front().first; stackPoint.pop_front();}
        else if (system.get_alphabet().find(c) != system.get_alphabet().end()) {
            cur.x = cur.x + std::cos(currentAngle); cur.y = cur.y + std::sin(currentAngle);
            if (system.draw(c)) {
                Line2D temp; temp.p1 = cur; temp.p2 = prev; temp.color = img::Color(color[0], color[1], color[2]);
