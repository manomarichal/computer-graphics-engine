...
namespace
{
	...
	void parse_rules(std::set<char> const& alphabet, std::map<char, std::list<replacement>>& rules, stream_parser& parser, bool parse2D)
	{
		parser.skip_comments_and_whitespace();
		...
		while (true)
		{
			...
			parser.assertChars("->");
			parser.skip_comments_and_whitespace();
			std::list<replacement > possibleRules;
			while (true) {
                parser.skip_comments_and_whitespace();
				...
				if (c == '=') {
					parser.skip_comments_and_whitespace();
					std::string chanche = parser.readQuotedString();
					r = {rule, std::stoi(chanche)};
                    parser.skip_comments_and_whitespace();
                    c = parser.getChar();
				}
				else r = {rule, 100};
				possibleRules.insert(possibleRules.end(), r);
				if (c == ';') continue;
				break;
			}
			rules[alphabet_char] = possibleRules;
			if (c == '}')
				break;
			else if (c != ',')
				throw LParser::ParserException("Expected ','", parser.getLine(), parser.getCol());
...
std::string const& LParser::LSystem::get_replacement(char c) const
{
	assert(get_alphabet().find(c) != get_alphabet().end());
	while (true) {
		for (const replacement &r: replacementrules.find(c)->second) {
			int i = rand() % 100;
			if (r.second - i > 0) {
				return r.first;
			}
